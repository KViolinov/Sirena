...
class _AddClusterFormScreenState extends State<AddClusterFormScreen> {
                ...
                ElevatedButton(
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      // Collect form data
                      final formData = {
                        'deviceNameController': _deviceNameController.text,
                        'device_description1':
  ...
  Future<void> sendFormDataToApi(Map<String, dynamic> formData) async {
    // Replace with your actual API endpoint and logic (optional)
    // final apiUrl = 'http://replace-with-your_api_url';
    ...
    final conn = await MySqlConnection.connect(ConnectionSettings(
      host: 'localhost',
      port: 3306,
      user: 'kvbbgcom_floodPrevention',
      password: 'kv0889909595',
      db: 'kvbbgcom_floodPrevention',
    ));

    try {
      // Prepare SQL statement (using `execute` with parameter binding)
      final sql = '''
      ...
      VALUES (_deviceNameController.text, _deviceDescriptionController.text,
       _deviceClusterController.text, , ?, ?, ?, ?, ?, ?, ?, ?)
    ''';

      final result = await conn.query(sql, formData.values.toList());

      // Handle successful insertion with null check
      if (result != null) {
        print('Device added successfully to database!');
      } else {
        print('No rows affected. Insertion might have failed.');
      }

      // You can optionally navigate back or show a success message here.
    } catch (e) {
      print('Error adding device to database: $e');
